...
import pygame
from pygame import Color, MOUSEBUTTONDOWN, RLEACCEL
...
__all__ = [
    'get_main_dir', 'split_thousands', 'sign', 'cw_corners', 'ccw_corners', 'distance', 'fill_gradient',
    'init_joystick', 'get_font', 'get_font_names', 'make_font', 'make_sysfont', 'draw_text', 'import_module',
    'calc_line', 'calculate_bezier', 'rot_center', 'plot_curve', 'load_image', 'wait_event',
]
...
def draw_text(image, text, pos=(0, 0), font='default', fg=(254, 254, 254), bg=(-1, -1, -1), rect_attr=('left', 'top')):
...
def wait_event(type_=MOUSEBUTTONDOWN):
    while 1:
        e = pygame.event.wait()
        if e.type == type_:
            return
